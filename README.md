...
[GitHub Actions]
   ...
   └─> sf-sequence.yml  : マージ順序チェック（Slack 通知付き・警告のみ）
```
...
- 順序が守られていない場合は PR の Annotations に**黄色いワーニングバナー**を表示し、Slack に通知する
...
### sf-validate.yml — PR 検証

PR 作成・更新をトリガーに、dry-run 検証を実行する。
...
> **注意:** `sf-sequence` のチェックは**ブロックなし・警告のみ**の動作のため、Required status checks には追加しない。PR の Annotations に警告バナーが表示された場合はレビュアーが確認して判断する。
